# Base class for creating rauc bundles
#
# Description:
# 
...
def write_manifest(bundle_path, d):
    import shutil
    import subprocess
...
fakeroot do_bundle() {
	...
	if [ ${RAUC_CASYNC_BUNDLE} -eq 1 ]; then
		...
		${STAGING_BINDIR_NATIVE}/rauc convert \
			...
			${CONVERT_ARGS} \
			${RAUC_BUNDLE_DEST}/bundle.raucb \
			${RAUC_BUNDLE_DEST}/casync-bundle.raucb
	fi
}
do_bundle[dirs] = "${RAUC_BUNDLE_DEST}"
do_bundle[cleandirs] = "${RAUC_BUNDLE_DEST}"
do_bundle[file-checksums] += "${RAUC_CERT_FILE}:False ${RAUC_KEY_FILE}:False"

addtask bundle after do_prepare_recipe_sysroot
